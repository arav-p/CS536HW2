...
class TCPStatsProcessor:
    ...
    def __init__(self, output_dir: str = "data"):
        """
        Initialize TCP stats processor.
    ...
    def process_test_results(self, results: Dict, server_id: str):
        ...
        Args:
            results: Results dictionary from IPerf3Client
            server_id: Unique identifier for this server test
        """
        ...
        for sample in samples:
            # samples are plain dicts (from asdict()) since iperf_client returns them that way
            record = {
            ...
            if tcp_info:
                ...
                # Fill with zeros if TCP info not available
    ...
    def load_stats(self, server_id: str) -> pd.DataFrame:
        """
        Load TCP stats from CSV file.

        Args:
            server_id: Server identifier

        Returns:
            DataFrame with TCP statistics
        """
        csv_path = self.output_dir / f"{server_id}_stats.csv"
    ...
    def compute_loss_signal(self, df: pd.DataFrame) -> pd.Series:
        """
        Compute loss signal from TCP stats.

        Args:
            df: DataFrame with TCP stats

        Returns:
            Series with loss signal (retransmits + lost packets)
        """
        loss = df['retransmits'].fillna(0) + df['lost'].fillna(0) + df['retrans'].fillna(0)
        return loss
    ...
    def get_summary_statistics(self, server_id: str) -> Dict:
        ...
        summary = {
            ...
            'rtt': {
                'min': df['rtt_us'].min() / 1000.0,  # Convert to ms
    ...
    def prepare_ml_dataset(self, server_ids: List[str]) -> pd.DataFrame:
        ...
        for server_id in server_ids:
            try:
                ...
                # Compute delta_cwnd (label for ML)
                df['delta_cwnd'] = df['snd_cwnd'].diff().fillna(0)

                # Add lagged features (use history)
    ...
    def split_train_test(self, df: pd.DataFrame, test_split: float = 0.3) -> tuple:
        ...
        For time series, we use the last portion as test set.
        ...
        """
        # For each server, split chronologically
        train_dfs = []
        test_dfs = []
...
if __name__ == "__main__":
    ...
    # This would be called after running iperf tests
    # Example: processor.process_test_results(results, "server1")
